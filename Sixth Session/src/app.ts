...
function myReverse<T>(arg: T[]): T[] {  //* une fonction qui aura un paramètre "T", et ensuite renvera un tableau de "T"
...
function reverse1<T>(arg: readonly T[]): T[] {
    return [...arg].reverse();
}
...
let reversedArr: number[] = reverse1(arr);
...
class B {
    private b: number = 3;  //* Cette propriété ne peut être accédée que dans la classe B
}
...
class C {
    public c: number = 3;  //* Cette propriété peut être accédée de n'importe où dans le code de notre application
}
