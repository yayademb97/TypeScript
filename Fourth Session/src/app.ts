...
function nextEXammple (a: string | boolean , b: string | number) {
...
function newExample (a: string | boolean, b: string | number){
    if (a === b) {
...
//* Exemple 3:
function thirdExample (a: string | Date){
...
function forthExample(a: string | string[]){
    if (Array.isArray(a)) {
...
function fifthExample(a: MouseEvent | HTMLInputElement){
    //! À Savoir que:
...
function typeNever(a: MouseEvent | HTMLInputElement){
    if ("azazazazaz" in a) {
...
function sithExample(a: any): a is Date{
    return a instanceof Date;
}

//? De cet exemple, en le testant au niveau de l'union de type
function myExample(a: Date | HTMLInputElement)
{
    if (sithExample(a)) {
